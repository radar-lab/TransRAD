...
class MaSAd(nn.Module):
    ...
    def forward(self, x: torch.Tensor, rel_pos, chunkwise_recurrent=False, incremental_state=None):
        ...
        k *= self.scaling
        qr = q.view(bsz, h, w, self.num_heads, self.key_dim).permute(0, 3, 1, 2, 4) #(b n h w d1) [b 4 160 160 24]
        ...
        output = output + lepe
        output = self.out_proj(output) #[b 160 160 96]
        return output
...
class RetBlock(nn.Module):
    ...
    def __init__(self, retention: str, embed_dim: int, num_heads: int, ffn_dim: int, drop_path=0., layerscale=False, layer_init_values=1e-5):
        ...
        self.pos = DWConv2d(embed_dim, 3, 1, 1)#use a 3 ×3 depthwise convolution at the beginning to encode relative position information implicitly. 
    ...
    def forward(
        ...
        if self.layerscale:
            ...
            x = x + self.drop_path(self.ffn(self.final_layer_norm(x))) #[b 160 160 96]
        return x
    
class PatchMerging(nn.Module):
    r""" Patch Merging Layer. output = H/2 and W/2
    ...
    def forward(self, x):
        ...
        x: B H W C
        '''
        x = x.permute(0, 3, 1, 2).contiguous()  #(b c h w) (192,160,160)
        x = self.reduction(x) #(b oc oh ow) (192,80,80)
        x = self.norm(x)
