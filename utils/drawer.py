...
def prepareFigure(num_axes, figsize=None):
    """Define figures and subplots with equal size by Lei."""
    assert num_axes <= 6, "The function supports up to 6 subplots."
    
    if figsize is None:
        figsize = (8, 6)  # Default figsize if not provided
    ...
    if num_axes == 3:
        fig, axes = plt.subplots(1, 3, figsize=figsize, width_ratios=[1, 4, 4],constrained_layout=True)
        return fig, axes
    ...
    if num_axes == 5:
        raise ValueError("Adding 5 subplots is not easy to organize.")
    
    if num_axes == 6:
        fig, axes = plt.subplots(2, 3, figsize=figsize, constrained_layout=True)
        return fig, axes.flatten()
    
def prepareFigure_old(num_axes, figsize=None):
...
def saveFigure(save_dir, name, dpi=300, quality=95):
    """ save the figure """
    plt.savefig(os.path.join(save_dir, name), dpi=dpi, bbox_inches='tight', pad_inches=1)
...
def drawInference(stereo_left_image, RD_img, RA_img, RA_cart_img, radar_nms_pred, \
                all_classes, colors, axes, radar_cart_nms=None,draw_RD=False,for_gt=False,draw_no_box=False):
    """ draw only boxes on the input images """
    if draw_no_box is False:
        color_black = [0., 0., 0.]
        for i in range(len(radar_nms_pred)):
            if draw_RD:
                bbox3d = radar_nms_pred[i, :6]
                if for_gt:
                    cls = int(radar_nms_pred[i, 6])
                else:
                    cls = int(radar_nms_pred[i, 7])
            else:
                bbox3d = radar_nms_pred[i, :4]
                cls = int(radar_nms_pred[i, 5])            
            color = colors[int(cls)]
            ### draw box
            mode = "box" # either "box" or "ellipse"
            ### boxes information added in the ground truth dictionary
...
def drawInference_rad(RD_img, RA_img, RA_cart_img, radar_nms_pred, \
