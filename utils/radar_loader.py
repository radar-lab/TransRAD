...
def readAnchorBoxes(anchor_boxes_file = "./anchors.txt",mode="3D"):
    ...
    if len(anchor_boxes) == 0:
        ...
        anchor_boxes = sortAnchorBoxes(anchor_boxes,mode)
    return anchor_boxes
...
def getSequenceNumbers(radar_dir, data_format):
    ...
    sequence_numbers = []
    for this_file in glob.glob(os.path.join(radar_dir, "*"+data_format[-1])):
...
############  radar  #####################################
def splitTrain(train_sequences,validate=True, ratio=0.1):
    """ Split train set to train and validate """
    total_num = len(train_sequences)
    validate_num = int(ratio * total_num)
    if validate:
...
def readSequences(config_data, mode, base = "RAD", pattern = "/*/*.npy"):
    """ Read sequences from train/test directories. """
    assert mode in ["train", "test"]
    assert base in ["RAD", "stereo_image", "gt"]
    filename = base + pattern
    if mode == "train":
        sequences = glob.glob(os.path.join(config_data["train_set_dir"], \
                            filename))
    else:
        sequences = glob.glob(os.path.join(config_data["test_set_dir"], \
                            filename))
    if len(sequences) == 0:
